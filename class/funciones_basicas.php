...
function limpiar(string $input): string {
...
//Método con rand()
function GenerateRandomString($length = 49) {
...
function getSubString1(?string $string, int $length = 40): string {
    if (empty($string)) {
        return '';
    }
    $stringSinEtiquetas = strip_tags($string);
    $longitudTotal = strlen($stringSinEtiquetas);
    if ($longitudTotal <= $length) {
        return $stringSinEtiquetas;
    } else {
        return substr($stringSinEtiquetas, 0, $length) . '...';
    }
}


function getSubString(?string $string, int $length = 40): string
{
    // Si el string es nulo o vacío, devuelve una cadena vacía
    if (empty($string)) {
        return '';
    }

    // Eliminar etiquetas HTML y obtener la longitud del texto sin etiquetas
    $stringSinEtiquetas = strip_tags($string);
    $longitudTotal = strlen($stringSinEtiquetas);

    // Cortar el texto y agregar puntos suspensivos si es necesario
    return match(true) {
        $longitudTotal <= $length => $stringSinEtiquetas,
        default => substr($stringSinEtiquetas, 0, $length) . '...'
    };
}
